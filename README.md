...
sing-box wrapper 每次运行还会检查 `route.rules` 中的 `resolve + ipv4_only`。如果 `vasma` 内核升级重写配置并移除该规则，wrapper 会先备份当前配置、生成并校验候选配置，再恢复规则并重启 sing-box；这避免 `zz` 在自动升级后重新出现 IPv4-only 漂移。
